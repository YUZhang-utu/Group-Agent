...
def write_tables(report, output):
    ...
    if report.get('kind') in ('full_library_conditions','condition_funnel'):
        ...
        if report.get('kind') == 'condition_funnel':
            pieces.append('<p><strong>Conditional counts:</strong> per-feature counts below concern only '
                'conformers passing the selected rule, not unconditional full-library feature totals. '
                'Necessary-condition rejects have no newly calculated pose.</p>')
    for kind in CLASSES:
