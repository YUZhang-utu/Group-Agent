...
### 2026-09-25 E071: composable pocket tool and general-agent requirements
- User reported a two-attempt failure for showing v001 residues within 5 A and requested general goal-directed operation/research rather than per-phrase patches. The supplied generic message does not identify the original failed call.
- Exposed the existing pocket_view operation to the bounded LLM tool schema, retaining exact catalog ligand identity, whole-residue selections, labels, exports and rollback. Enabled requested objects. Accepted the pymol_agent prefix with or without slash.
- Passed bounded session history into the inner PyMOL planner; previously only the outer router received it. Surfaced underlying errors in final failure messages. Avoided unrelated component parsing for non-interaction programs.
- Removed the provisional phrase-specific shortcut after the user clarified the architectural requirement. Documented the unimplemented unified research/tool loop and literature integration in to_human/E071_CHAT_AGENT_ARCHITECTURE.md.
- Validation: 55 focused tests passed, one real-PyMOL test skipped; English guard and diff check passed. Planner and desktop API fixtures do not establish live-provider understanding or workstation rendering. No claim of a general research agent implemented this turn.

### 2026-09-25 E072: domain tool-feedback loop and library connection
- Replaced ordinary Chat one-shot routing with a bounded model/tool/result loop; retained explicit commands and reused validated workflow executors. Added owned task/report reads, discovered artifact paging, exact molecule/conformer provenance lookup through runtime.search.batch, Europe PMC abstract retrieval and viewer inspection.
- Preserved asynchronous scientific jobs and existing compute policies. Recorded queued/resumed job IDs, blocked repeated mutations and further mutation while dispatch is pending, and persisted per-step evidence/error traces with authenticated UI download.
- Protocol: experiments/E072-domain-agent.md. Guide and workstation prompts: to_human/E072_DOMAIN_AGENT.md. No full-library run or reconstruction was launched.
- Validation: full-suite run had 564 passing tests, three skips and one newly added resume fixture failure. Corrected the fixture to follow the existing decision schema; all eight domain tests then passed. After the final prompt-contract and authenticated trace-download checks, all 18 domain/Chat tests passed. Thus all 565 collected runnable tests passed across these runs. JavaScript syntax, English-content and whitespace checks passed.
- These are local executor/fixture checks, not live LLM quality, outbound literature networking or workstation PyMOL rendering acceptance. No claim of superiority over Codex. Automatic agent-loop recovery and full-text literature review remain unimplemented; scientific job resume remains available.

### 2026-09-25 E073: source-aware PDB/diversity/consensus/search integration
- Added structure_workflow to the domain agent: actual stage state, source-run/task relationships, reference options, diversity proposals and registered similarity artifacts. Missing readiness or failed child reports cannot advertise a ready consensus transition.
