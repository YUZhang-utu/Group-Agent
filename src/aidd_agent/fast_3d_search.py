...
def exhaustive_candidates(corpus, raw_query, mean, std, output, budget=10000, chunk_size=65536):
    """Compare every stored conformer; bounded exact Top-K with global-ID ties."""
    ev.require(raw_query.shape == mean.shape == std.shape == (60,), "Invalid descriptor shape")
    ev.require(np.isfinite(raw_query).all() and np.isfinite(mean).all() and np.isfinite(std).all()
               and np.all(std > 0) and budget > 0 and chunk_size > 0, "Invalid exhaustive settings")
    end = 0
    for row in sorted(corpus.rows, key=lambda r: r['global_id_start']):
        ev.require(row['global_id_start'] == end and row['conformers'] > 0, "Catalog ranges have gaps or overlap")
        end += row['conformers']
    ev.require(end == corpus.total and end > 0, "Catalog coverage differs from total")
    query = (raw_query - mean) / std
    best, _, timing = ev.exact_truth(corpus, [query], [b''], mean, std,
                                    min(budget, corpus.total), chunk_size)
    distances, ids = best[0]
    ev.require(len(ids) == min(budget, corpus.total) and np.isfinite(distances).all(), "Incomplete exact retrieval")
    molecules, _ = corpus.fetch(ids)
    np.savez(output, global_ids=ids, molecule_ids=molecules, squared_l2=distances)
    return dict(mode="exhaustive", candidates=len(ids), total_conformers=corpus.total,
                compared_conformers=end, coverage_fraction=1.0, all_conformers_compared=True,
                candidate_budget=budget, chunk_size=chunk_size, search_seconds=timing['wall_seconds'],
                timing=timing, exact_reference_scan=False,
                scope="Actual exhaustive query computation, not an offline reference scan; budget retention is not chemical rejection")
...
def run(args):
    ...
    with (batch / "run.lock").open("r") as lock:
        ...
        if args.fresh_retrieval or exhaustive:
        ...
        try:
            ...
            for label, qid in query_specs:
                ...
                def operation():
                    ...
                    if exhaustive:
                        ...
                        retrieval = exhaustive_candidates(corpus, np.load(source / label / "query/usrcat.npy", allow_pickle=False),
                                                          mean, std, schedule)
                    elif args.fresh_retrieval:
                        schedule = root / "candidates.npz"
                    ...
                    if exhaustive:
                        check = dict(passed=None, status="not_comparable_different_candidate_set")
                        annotation_check = dict(check)
                    else:
            ...
            lines += ["", "Every conformer participated in exhaustive coarse retrieval; changed candidates are not compared to ANN baseline outputs." if exhaustive else "All Gaussian arrays and E031 scores/assignments/ranks passed equivalence checks.",
                      "Exhaustive descriptor scanning is included in query execution time." if exhaustive else "Preflight/index loading and final equivalence checks are separate. No exact full-library scan.",
...
def main():
    ...
    p.add_argument("--exhaustive", action="store_true", help="Compare all stored conformer descriptors before budget selection")
