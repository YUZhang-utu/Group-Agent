...
## E034 engineering handoff — 2026-09-18
E033 results support a provisional coarse descriptor setting but do not yet establish target-specific recall or pose quality. E034 now implements that next measurement without reusing library-panel candidates for WEE1. The old77.99-second E031 denominator is removed from the legacy runner; the new runner compares same-query same-run refinement and sidecar timings and declines a gate on partial chunk reuse. Expanded-library mmap readers also require a larger process file-descriptor budget; startup checks now handle it before refinement. Full workstation evaluation remains pending; no new scientific result is inferred from the166passing local checks.
...
## E035 completed evidence and E036 performance hypothesis — 2026-09-18
User-reported million-conformer stress equivalence is exact (6M comparisons, zero score/assignment changes), and full E031 calls are1.448/1.909s. This supports the opt-in fast annotation engine, not biological superiority. Source-grouped molecule Top100 intersections6/9;39/41poses await review. E034 coarse has only5tasks for16workers; refine26–28tasks leaves potential tail imbalance. E036 tests finer500/64chunks with unchanged scoring and candidate effort. Scale validation now has8independent workers by default. Local correctness194passed/2skipped, but real scheduling speedup unmeasured. Cumulative worker CPU-like compute times must not be interpreted as elapsed wall time. Whole-index query execution is separated from one-time/reference evaluation costs; currently restricted to two calibrated WEE1 queries.

## E037 mechanism and limits — 2026-09-18
The reference Gaussian path constructs the entire deterministic pair-seed list before taking512. Returning at512unique seeds removes an unused suffix without changing candidate poses or their ordering. This is an exact-work-elimination hypothesis, unlike lowering the seed budget. Local12synthetic cases confirm exact prefixes; oversubscribed cases accelerate generator work1.27–9.69x, cases below cap show no material benefit. Full Gaussian array equality tested; overall search speedup is still unknown and depends on actual seed cardinalities and scoring cost. E037 factorial workstation suite separates scheduling from generation effects, reverse order reduces but does not eliminate timing confounding. Profiles only sample first coarse/refine chunks; exported E035 poses remain unreviewed.
