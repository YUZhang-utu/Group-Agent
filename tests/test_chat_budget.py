...
def test_chat_budget_queues_owned_search_and_next_page(tmp_path):
    ...
    app.router=lambda *args:dict(intent='budget',message='',request='',task_id=job['id'],budget=dict(retrieval_molecules=500000,export_molecules=100000))
...
def test_budget_search_is_target_generic(tmp_path,monkeypatch):
    ...
    result=execute('consensus_budget',source,tmp_path/'out',{},dict(search=dict(batch=str(tmp_path))),True)
